...
use crate::page::Page;
use crate::{buffer_pool_manager::BufferPoolManager, PageId};
...
*/
#[derive(Debug)]
struct ExtendibleHashTable<K, V> {
    name: String,
    ...
    V: Copy + Clone + Debug + Serialize + DeserializeOwned,
{
    pub fn new(
        ...
        let locked_buffer_pool_manager = Arc::clone(&buffer_pool_manager);
        let mut locked_buffer_pool_manager = locked_buffer_pool_manager.lock().unwrap();
        // TODO: what if BPM is not able to create new page
        let header_page = locked_buffer_pool_manager.new_page().unwrap();
    ...
    pub fn insert(&self, key: K, value: V) -> Result<(), ExtendibleHashTableError> {
        ...
        let (mut directory, directory_page) = match header.get_directory_page_id(directory_index) {
            Some(directory_page_id) => {
                let directory_page_arc = self.fetch_page(*directory_page_id).unwrap();
                let directory_page = directory_page_arc.lock().unwrap();

                (
                    ExtendibleHTableDirectoryPage::from(&directory_page),
                    Arc::clone(&directory_page_arc),
                )
            }
            None => {
                let mut buffer_pool_manager = self.buffer_pool_manager.lock().unwrap();
                let new_page_arc = buffer_pool_manager.new_page().unwrap();
                let new_page = new_page_arc.lock().unwrap();
        ...
        self.insert_internal(key, value, &mut directory, directory_page)?;
    ...
    fn insert_internal(
        ...
        directory_page: Arc<Mutex<Page>>,
    ) -> Result<(), ExtendibleHashTableError> {
        let insertion_key_hash = hash_string(key.to_string());
        let bucket_index = directory.hash_to_bucket_index(insertion_key_hash);
        let (mut bucket, bucket_page) = match directory.get_bucket_page_id(bucket_index) {
            Some(bucket_page_id) => {
                let bucket_page_arc = self.fetch_page(*bucket_page_id).unwrap();
                let bucket_page = bucket_page_arc.lock().unwrap();
                let data = bucket_page.get_data();

                (
                    ExtendibleHTableBucketPage::from_bytes(data.as_slice()),
                    Arc::clone(&bucket_page_arc),
                )
            }
            None => {
                let mut buffer_pool_manager = self.buffer_pool_manager.lock().unwrap();
        ...
        if !bucket.is_full() {
            ...
            let mut buffer_pool_manager = self.buffer_pool_manager.lock().unwrap();
            let new_page = buffer_pool_manager.new_page().unwrap();
            ...
            for entry in all_entries {
                let key = entry.0;
                let value = entry.1;
                self.insert_internal(key, value, directory, Arc::clone(&directory_page))?
            }
    ...
    pub fn remove(&self, key: K) -> Result<(), ExtendibleHashTableError> {
        ...
        let mut directory_page = directory_page.lock().unwrap();
        ...
        let bucket_page = self.fetch_page(bucket_page_id).unwrap();
        let mut bucket_page = bucket_page.lock().unwrap();
    ...
    fn fetch_page(&self, page_id: PageId) -> Option<Arc<Mutex<Page>>> {
        let mut buffer_pool_manager = self.buffer_pool_manager.lock().unwrap();
        let page = buffer_pool_manager.fetch_page(page_id);

        page.map(|p| Arc::clone(&p))
    }
}
#[cfg(test)]
mod tests {
